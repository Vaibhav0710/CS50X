...
/*
Take as argument a char* or a string which is the dictionary file to open up and read from
Use fopen to open file
Return true if executed successfully
If return value is NULL, return false (memory allocation error)
Use fscanf to read the strings in the file till EOF (end of file)
Use malloc to create and allocate memory for each new node
If it returns NULL, return false
Copy word into node using strcpy
Use hash to take the words and return a hash value/index
Insert node into hash table by setting pointer to consecutive nodes
Count the number of words being input
*/
